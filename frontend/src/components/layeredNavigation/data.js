export const layeredNavigationData = [
  {
    label: "Category",
    items: [
      "Protein (12)",
      "Stacks &amp; Bundles (9)",
      "Clearance &amp; Overstock (8)",
      "Whey Protein Isolate (7)",
      "Workout Stacks (7)",
      "Protein Clearance (7)",
      "Fat Loss (4)",
      "Pre-Workout (4)",
      "Thermogenic Fat Burners (3)",
      "Glutamine (2)",
      "Mass Gainers (2)",
      "BCAAs &amp; EAAs (2)",
      "Health &amp; Wellness (2)",
      "Muscle Building Stacks (2)",
      "Immune Support (2)",
      "Vegan Protein (2)",
      "Keto (2)",
      "High Stimulant (2)",
      "Fat Loss Pre-Workouts (2)",
      "1000+ Calorie Gainers (2)",
      "Whey Protein (1)",
      "Workout Accessories (1)",
      "Shaker Bottles (1)",
      "Other Products (1)",
      "Sexual Health (1)",
      "Fat Loss Stacks (1)",
      "Women's Stacks (1)",
      "Nootropics (1)",
      "Health &amp; Wellness Clearance\n                            (1)",
    ],
  },
  {
    label: "Brand",
    items: [
      "Perfect Sports (39)",
      "MusclePharm (3)",
      "Muscle &amp; Strength (3)",
      "AllMax Nutrition (2)",
      "NOW Foods (2)",
      "USN (2)",
      "BPI Sports (1)",
      "MAN Sports (1)",
      "Nutrex (1)",
      "Force Factor (1)",
      "FitMiss (1)",
      "Obvi (1)",
    ],
  },
  {
    label: "Price",
    items: [
      { price: "$10.00", num: "4" },
      { price: "$10.00", num: "7", price2: "$20.00" },
      { price: "$20.00", num: "7", price2: "$30.00" },
      { price: "$30.00", num: "6", price2: "$40.00" },
      { price: "$40.00", num: "3", price2: "$50.00" },
      { price: "$50.00", num: "3", price2: "$60.00" },
      { price: "$60.00", num: "5", price2: "$70.00" },
      { price: "$70.00", num: "4", price2: "$80.00" },
    ],
  },
  { label: "Average Rating" },
  {
    label: "Product Flavor",
    items: [
      "Fruit Punch (6)",
      "Vanilla Ice Cream (6)",
      "Chocolate Ice Cream (5)",
      "Chocolate Peanut Butter Ice Cream\n                            (5)",
      "Cookies 'n Cream Ice Cream (5)",
      "Orange Gummy Bears (5)",
      "Strawberry Ice Cream (5)",
      "Banana (3)",
      "Blue Raspberry (3)",
      "Chocolate Wafer Crisp (3)",
      "Cookies 'n Cream (3)",
      "French Vanilla (3)",
      "Pineapple Mango (3)",
      "Triple Rich Dark Chocolate (3)",
      "Unflavored (3)",
      "Candy Cane Ice Cream (2)",
      "Chocolate Monkey (2)",
      "Chocolate Peanut Butter (2)",
      "Intense Hawaiian Pineapple Candy\n                            (2)",
      "Intense Peach Rings Candy (2)",
      "Intense Watermelon Candy (2)",
      "Marshmallow Hot Chocolate (2)",
      "Milk Chocolate (2)",
      "Pumpkin Spice Latte (2)",
      "Salted Caramel (2)",
      "Ultra Orange (2)",
      "Candy Tarts (1)",
      "Chocolate Delight (1)",
      "Chocolate Fudge Brownie (1)",
      "Chocolate Ice Dream (1)",
      "Chocolate Milk (1)",
      "Chocolate Peanut Candy (1)",
      "Cookies N' Cream (1)",
      "Grape (1)",
      "Green Apple (1)",
      "Intense Blue Raspberry (1)",
      "Lemon Drop (1)",
      "Lemon Lime (1)",
      "Mango (1)",
      "Orange Mango (1)",
      "Peach Bellini (1)",
      "Peach Rings (1)",
      "Puck 'n Sour Watermelon Candy (1)",
      "Rainbow Sherbet (1)",
      "Sour Cherry (1)",
      "Southern Sweet Tea (1)",
      "Strawberry (1)",
      "Strawberry Daiquiri (1)",
      "Swedish Sharks Candy (1)",
      "Vanilla (1)",
      "Vanilla Chai (1)",
      "Vanilla Cupcake (1)",
      "Watermelon (1)",
    ],
  },
  {
    label: "Product Size",
    items: [
      "30 Servings (10)",
      "5lbs (7)",
      "1lb (3)",
      "4.4lbs (3)",
      "60 Capsules (3)",
      "1 Size (2)",
      "1.6lbs (2)",
      "120 Capsules (2)",
      "2lbs (2)",
      "60 Tablets (2)",
      "1 Shaker (1)",
      "1,000g (1)",
      "1.5lbs (1)",
      "100 Tablets (1)",
      "10lbs (1)",
      "30 Capsules (1)",
      "300 Servings (1)",
      "31 Servings (1)",
      "350g (1)",
      "36 Servings (1)",
      "3lbs (1)",
      "40 Servings (1)",
      "400g (1)",
      "4lbs (1)",
      "50 Servings (1)",
      "5mg 60 VCaps (1)",
      "700mL (1)",
      "75 Capsules (1)",
      "90 Capsules (1)",
      "90 Softgels (1)",
      "96 Capsules (Exp. 04/22) (1)",
    ],
  },
  {
    label: "Form Factor",
    items: [
      "Powder (33)",
      "Capsule (6)",
      "Tablet (3)",
      "Liquid (1)",
      "Softgel (1)",
      "Vegetarian Capsule (1)",
    ],
  },
];
